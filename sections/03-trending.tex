...
The development is always with respect to a time horizon, that is, the amount of time passed during the development. 
In general, trending can be assessed for different horizons $l$ separately, and the relevance of the considered horizons has to be determined based on the question at hand.
...
For forecasting, the notation is similar: Let $x_{t | \tau}$ denote the forecast for forecast time $t$ and issue time $\tau$. 
...
In measurement analysis, the distinction between forecast and issue time is not necessary, as the measurement is typically available at the time. 
Let, thus, $x_t$ denote the test method measurement for time $t$. 
The computation
\begin{equation}
  (x_{t} - x_{t-l})_{t=l}^T \label{eq:diffxl_measure}
\end{equation}
yields the change by the test method. 
It is computed purely by the test method without the gold standard $y_t$ to analyse whether the changes by gold standard and test method are consistent.
...
\end{equation}
The enumerator counts the number of same-sign-changes, while the denominator is the number of all points.
