...
Thus, the gold standard measurements are not available if the quality of the test method is sufficient. \improvement{Änderung des Testgeräts an sich beurteilen}
...
$\\
        \bottomrule
    \end{tabularx}
    \caption{Computation of the predicted change in the different applications. Signals $x_{t | \tau}$ refer to values issued at $\tau$ with a target time $t$. Note the different relations between the issue and target time for nowcasts and forecasts. \todo{$T-l$}}
    \label{tab:notation}
\end{table}
...
\end{equation}
We refer to this estimator as the trending ratio of the prediction and set $\mathcal{T} = \{1, \dots, T-l\}$. \todo{Durchwalken; nennen, warum ratio; Anteil concordant points an allen, deswegen nennen wir es ratio}
...
Two simple measures that focus on specific areas of interest \todo{welche areas} are the positive and negative trending ratios $\accp$ and $\accm$, respectively.
They are defined as
\begin{align}
...
All measures can also be evaluated as a rolling estimate to detect changes in performance over time. \todo{Klarer, warum man das soll. Wir wollen eigentlich trending ratio jetzt wissen. }
A rolling estimate is a sequence of estimates, each computed on a length-$w$-window of the data.
For the trending ratio, a rolling estimate with a backward-looking \todo{Klarer, was heißt backward-looking} window is given by
\begin{equation*}
...
\end{equation*}
\todo{Was heißt w? Großes w weniger fokussiert auf current situation; kleines w lokaler}
...
\todo{Ohne das negative; groß E; für genau ein $x$ anschauen, geht nicht mit abzählen; deswegen Schätzen der Dichte}
...
\begin{figure}
    ...
    \end{subfigure}
    \caption{Resulting conditional trending plot for different bandwidth selection processes. Cross-validation least squares takes a considerably larger computation time. It does not converge for the first data set and yields a bandwidth too small for the second data set. The rule of thumb is the fastest method but tends to oversmooth. The cross-validation maximum likelihood method yields a more reasonable bandwidth with moderate computation time. \todo{$\varepsilon$ hinzufügen}}\label{fig:trending-cond-prob-bw}
\end{figure}
