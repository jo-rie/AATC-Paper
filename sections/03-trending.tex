...
Thus, the predicted change can be computed with the knowledge of time $t$. Therefore, the trending information in the following section can be used in practice at time $t$.
...
\end{align}
They give the probability of a correct prediction of the direction of change, given that the signal direction is positive or negative, that is, $P(\diffxrv \diffyrv > 0 | \diffxrv > 0)$ and $P(\diffxrv \diffyrv > 0 | \diffxrv < 0)$.
...
Using an exclusion area around the zero point, as further defined below, is a straightforward and highly interpretable extension of the measures of Section~\ref{subsec:trending-measures} \citep[see, e.g.,][]{Saugel2015,Critchley2010}.
...
New samples are drawn with replacement from the dataset, and the confidence interval is then computed based on those bootstrap samples.
...
Probabilistic predictions include a point estimate and information on the prediction uncertainty and quantiles simultaneously.
...
The \ac{bs} is a widely used scoring rule for dichotomous probabilistic forecasts~\citep{Brier1950} and is defined as
\begin{equation*}
    BS (\mathbf{p}, \mathbf{z}) = \frac{1}{T-l+1} \sum_{t=l}^{T} (p_t - z_t)^2.
\end{equation*}
Lower values indicate a higher probabilistic trending ability of the method.
...
The basic paradigm of probabilistic forecasting is to maximize sharpness, subject to calibration~\citep{Gneiting2014}.
