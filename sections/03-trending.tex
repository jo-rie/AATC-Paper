...
Similarly, we call a measurement, nowcast, or forecast a \textit{prediction} for the quantity of interest $y$. \discuss{prediction a good word?}
...
\unsure{\cite{perrino1998intraoperative}}
...
Analyzing the number of points in the green quadrants versus the red quadrants boils down to assessing the probability of trending $P(\diffxrv \diffyrv > 0)$, where $\diffyrv$ and $\diffxrv$ denote random variables for new elements of $\diffy$ and $\diffx$, respectively. \discuss{vielleicht genauer: wofür diese zwei ZV stehen}
Note that $z_1 z_2 > 0$ if and only if $\sign(z_1) = \sign(z_2)$ ($z_1, z_2 \in \R \setminus \{ 0 \}$).
\unsure{der Fall: z1 = z2 = 0?}
...
\discuss{was heisst unbalanced konrket? und vielleicht kappa auch irgendwie anders einführen}
...
\discuss{kann man vielleicht mu für verschiedene Varianten von Exclusion Area ausrechnen als Vergleich? evtl. im Anhang?}
In addition to these straight shapes, exclusion areas could also be determined based on an error model of the components.
<<<<<<< Updated upstream
For example, one could exclude points likely to be in another quadrant or within a specific quantile of a distribution around the zero point. \unsure{vielleicht genauer bei der Verwendung von Quantilen}
...
=======
For example, one could exclude points likely to be in another quadrant or within a specific quantile of a distribution around the zero point. \discuss{vielleicht genauer bei der Verwendung von Quantilen}
These shapes are theoretically appealing, but the interpretation of the resulting estimators gets ambiguous.
>>>>>>> Stashed changes
...
\discuss{wollen wir hier den Fass aufmachen?}
...
\done{Asymmetrie vielleicht irgendwo oben als Begriff einführen}
...
\done{berücksichtigen die hier angeweneten Verfahren die Abhängigkeit?}