...
Figure~\ref{fig:trending-cond-prob-bw} shows the resulting conditional trending plots for the three well-known selectors, rule-of-thumb, cross validation maximum likelihood and cross validation least squares.
\todo{Add analysis.}

Figure~\ref{fig:trending_cond_prob} shows the results for the different snythetic datasets again, but only with the best-balanced cross validation least squares estimator.
...
\begin{figure}
    ...
    \begin{subfigure}{.48\textwidth}
        \missingfigure{Influence of bw method; Data set 1}
    \end{subfigure}
    \begin{subfigure}{.48\textwidth}
        \missingfigure{Influence of bw method; Data set 2}
    \end{subfigure}
    \caption{Resulting conditional trending plot for different bandwidth selection processes.}\label{fig:trending-cond-prob-bw}
\end{figure}

\begin{figure}
    \centering
    \missingfigure{Trending plot for two different simulation examples -> the two data samples of bootstrapping?}
%    \includegraphics{}
    \caption{}
    \label{fig:trending_cond_prob}
\end{figure}
...
The considered datasets are again the ones outlined in Appendix~\ref{sec:app-trending-data-generation}.
The results in terms of coverage of the true values are shown in Table~\ref{tab:trending_bootstrap}.
The computation times per method and dataset are shown in Table~\ref{tab:trending_bootstrap_time}.

\begin{table}
    \centering
    \missingfigure{Bootstrap results}
    \caption{}
...
\begin{table}
    ...
    \missingfigure{Bootstrap results}
    \caption{}
    \label{tab:trending_bootstrap_time}
\end{table}
...
\begin{itemize}
    \item We use the alogrithms provided by Python's \verb|scipy|-package~\parencite{Virtanen2020}.
    \item Two simulation setups
    \item Take different $T$
    \item Compute "true value" based on very large sample.
    \item Compute share of bootstrap confidence intervals that actually covered the true value for different $B$.
    \item Results in "double table": columns $T$, main rows $B$, undercategory: bootstrap methods.
\end{itemize}