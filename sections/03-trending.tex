...
The use of those estimators could encourage the predictors to exploit imbalances in the number of positive and negative changes. 
...
In applications, signal data is often not available for all time steps, for example, due to technical problems or delays in data transfer (see the examples in Sections~\ref{sec:application-covid} and~\ref{sec:application-eda}).
We refer to time steps, for which either signal or true values or both are unavailable, as missing values.
Data pairs with missing values can be excluded from the set $\mathcal{T}$ for the computation of the measures, and the measures are computed normally. 
...
In this case, the measures should be interpreted cautiously, and
inspection of missing data should accompany applications with missing data, for example, through a visual assessment.
In the applications in Sections~\ref{sec:application-covid} and~\ref{sec:application-eda}, only a few missing values occur, and we give information on the missing data by inspecting the missing data as lists.
...
In the example plot in Figure~\ref{fig:trending_basic_4q_excl_box}, only point 1 is excluded.
...
For those two methods, points 1 and 7 in Figure~\ref{fig:trending_basic_4q_excl_axis} or 1, 6, and 7 are excluded in Figure~\ref{fig:trending_basic_4q_excl_cross}, respectively.
...
The \ac{bca} is the only method holding the confidence level also for small datasets while increasing the computation time only moderately for larger datasets.
...
In addition, the plot facilitates the comparison of various methods in a single plot, and asymmetries of $P(\diffxrv \diffyrv > 0 | \diffx = x)$ with respect to $x$ in the trending ability can be detected.
...
The multivariate \ac{kde} takes as modeling parameters a kernel and a bandwidth selector. 
...
While the kernel is typically Gaussian, various methods for the bandwidth selector are available with different strengths and weaknesses.
