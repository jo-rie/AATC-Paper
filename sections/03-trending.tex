...
As noted above, a prediction is trending perfectly if all occurred changes are predicted correctly, that is, the sign of all elements of $\diffx$ and $\diffy$ coincide.
 \unsure{vielleicht nicht change sondern direction of the change}
...
Analyzing the number of points in the green quadrants versus the red quadrants boils down to assessing the probability of trending $P(\diffxrv \diffyrv > 0)$, where $\diffyrv$ and $\diffxrv$ denote random variables for new elements of $\diffy$ and $\diffx$, respectively. \unsure{vielleict genauer: wofür diese zwei ZV stehen}
...
\unsure{was heisst unbalanced konrket? und vielleicht kappa auch irgendwie anders einführen}
...
If there is noise or non-systematic effects present in true value or nowcasts, those points are likely to be driven by noise to a large amount. \unsure{anstatt nowcasts eher predictions}
...
\unsure{kann man vielleicht mu für verschiedene Varianten von Exclusion Area ausrechnen als Vergleich?}
...
\unsure{wollen wir hier den Fass aufmachen?}
...
\unsure{Allgemeine Frage: können wir hier von Wahrscheinlichkeit sprechen? }
...
\unsure{Asymmetrie vielleicht irgendwo oben als Begriff einführen}
...
\unsure{berücsksichtigen die hier angeweneten Verfahren die Abhängigkeit?}