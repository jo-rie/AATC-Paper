...
If the forecasts or nowcasts are given as quantiles, $p_t$ can be computed by interpolations between the quantiles.
Let $q_p$ denote the quantiles for forecast time $t+l$ for even-spaced probabilities $p \in \{1/P, \dots, (p-1) / P\}$ ($P \in \N \setminus \{1, 2\}$) and $y_t$ the true value at time $t$.
Then, the probability $\bar{p}_t$ of a negative change is between $p^{\star}$ and $p^{\star} + 1/P$ for
\begin{equation*}
    p^{\star} = \max \{p \in \{1/P, \dots, (p-1) / P\}: q_p \leq y_t\} , \text{if} q_{1/p} \leq y_t \leq q_{1 - 1/p}.
\end{equation*}
The quantiles do not determine the location within the interval.
Under the assumption of a uniform distribution within the quantile interval, the probability of a negative change is
\begin{equation*}
    \bar{p}_t = \frac{y_t - q_{p^\star}}{P (q_{p^{\star} + 1} - q_{p^{\star}})} + p^{\star}.
\end{equation*}
...
\bar{p}_t = \begin{cases}
    \max \{\frac{1}{P} - \frac{q_{p^\star} - y_t}{P (q_{p2/P} - q_{1/P})}, 0\} &, \text{if } y_t < q_{1/p}, \\
    \min \{\frac{1}{P} - \frac{y_t - q_{(P-1)/P}}{P (q_{(P-1)/P} - q_{(P-2)/P})}, 1\} &, \text{if } y_t > q_{1 - 1/p}, \\
    \frac{y_t - q_{p^\star}}{P (q_{p^{\star} + 1} - q_{p^{\star}})} + p^{\star} &, \text{otherwise.}
\end{cases}
\end{equation*}
