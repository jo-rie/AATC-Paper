...
Development always has respect to a time horizon, that is, the amount of time passed. 
...
In a four-quadrant plot, the occured changes are plotted with the predicted changes, that is, $(\diffyt, \diffxt)$ for $t = 1, \dots, T$.
...
We refer to time steps for either signal or true values, or both are unavailable as missing values.
...
In particular the prediction models are likely to have a noise component and, thus, $\diffx$ should have an exclusion area.
...
The size estimation can also be based on a proportion of the total variance or the total range of the data; for example, the size is twice the 10\% quantile of absolute values in each component and thus excludes the smallest 10 \% of points.
