...
Methods: We reviewed and developed visual techniques and quantitative measures to assess ATC. 
Extensions for noisy data and estimation uncertainty were implemented using bootstrap confidence intervals and exclusion areas. 
...
The proposed methods effectively evaluated ATC across different applications. 

Conclusions: 
The developed ATC assessment methods offer a comprehensive toolkit for evaluating the ATC of measurements, nowcasts, and forecasts. 
These techniques provide valuable insights into model performance, complementing traditional accuracy measures and enabling more informed decision-making in various fields, including public health, healthcare management, and medical diagnostics.
}
\keywords{}
...
In forecasting and nowcasting the evaluation of performance or goodness is usually based on statistical methods quantifying the accuracy such as the \ac{rmse}, probabilistic scoring rules, and calibration measures~\citep{Gneiting2007, Gunther2021, Wolffram2023}, see also~\citet{SteyerbergEwoutW.2010AtPo} for an overview in the medical context together with less known measures.
However, although techniques like ROC curves for binary direction models or more general concordance measures \citep[see, e.g., ][]{Goenen2005} are used, there is no general measure directly framed to assess the method's \ac{atc}.
...
When evaluating the performance or goodness of a measurement method like in classical method comparison studies, comparative statistics such as Bland-Altman analysis~\citep{Bland1986} and the percentage error \parencite{Critchley1999} are commonly used. In this strand of literature, the best way to assess the \ac{atc} of measurement methods is a field of active ongoing research~\citep{Critchley20101180, CRITCHLEY2011536, Saugel2015, Saugel2018, Hiraishi2021}.
...
We discuss practical implementation and interpretation of the measures, thus providing the basis for communicating model limitations to forecasters, nowcasters, and public health officials.
Ready-to-use code is available on \url{https://github.com/jo-rie/aatc}.
...
We base the assessment of \ac{atc} on the measured/observed/true and the predicted changes in a value of interest over a time horizon $l$.
...
While the computation for measurements is straightforward and well-established, we develop the framework for nowcasts and forecasts in the following sections.
...
A simple yet insightful method is the four-quadrant plot, which is well-established in measurement analysis and can be extended to nowcasts and forecasts~\citep[see, e.g., ][]{Perrino1998, Saugel2015}.
...
Analyzing the number of points in the green versus red quadrants is a standard approach in the \ac{atc} assessment of measurement data~\citep{Critchley2010, Saugel2015}, which we extend here to forecasts and nowcasts.
...
Confidence intervals can account for the estimation uncertainty of the measures above, an approach not yet applied in the literature.
...
The estimators described above provide information on the probabilities $P(\diffxrv \diffyrv > 0 | \diffxrv \diffyrv \notin E)$, $P(\diffxrv \diffyrv > 0 | \diffxrv > 0, \diffxrv \diffyrv \notin E)$, and $P(\diffxrv \diffyrv > 0 | \diffxrv < 0, \diffxrv \diffyrv \notin E)$.
Notably, the visual analysis of these probabilities has not yet been addressed in the literature.
...
In this section, we develop \ac{atc} assessments for probabilistic measurements and nowcasts, an approach not yet explored in the literature.
...
Traditional methods often focus on point or distributional accuracy of hospitalization rates, but understanding the direction and reliability of changes is crucial for effective decision-making. The \ac{atc} assessment offers easy-to-interpret insights and helps determine not only if rates are rising or falling, but also how confidently we can make this determination.
For instance, if hospitalization rates are rising, public health measures may need to be tightened. Conversely, falling rates might justify loosening restrictions. The \ac{atc} assessment's ability to reveal asymmetries is especially valuable, as it can show whether certain models are more adept at recognizing decreases than increases, or vice versa. This information, which is not readily apparent from traditional methods, can significantly impact the interpretation of nowcasts and subsequent policy decisions for public health officials.
...
While traditional methods provide point estimates, \ac{atc} assessment offers a simple, intuitive way to evaluate model performance and directional accuracy. This approach is particularly valuable for management, as it facilitates easy comparisons between expected workload and recent shifts. For instance, if staff was near capacity during the last shift, \ac{atc} assessment can clearly indicate whether an increase in patient admissions is likely, allowing management to proactively adjust resources. This directional insight, often hidden in conventional methods, enables more informed and timely decision-making.
...
\begin{table}
    ...
    \caption[ATC ratios for the forecasts of patient admission to an emergency department.]{\Ac{atc} ratio $\acc_+$, positive \ac{atc} ratio $\accp_+$, and negative \ac{atc} ratio $\accm_+$ for the models for the horizons 72 hours and seven days in the forecasting of patient admissions to an emergency department with an exclusion zone equal to the axis indicated by the subscript $+$. There are 4030 positive changes and 4051 negative changes for the horizon of 72 hours and 4042 and 3961 for the horizon of 7 days.}
...
While \ac{atc} assessment is well-established in measurement analysis, our new methods extend its application, revealing insights such as asymmetries. 
This extension enhances the utility of \ac{atc} assessment beyond its conventional use, offering a more comprehensive evaluation of model performance.
...
For public health officials and forecasters analyzing methods, we recommend presenting four-quadrant plots, \ac{atc} ratios and the conditional \ac{atc} plots as a fundamental approach. 
This presentation makes a model's \ac{atc} easily interpretable and and reveals performance asymmetries.
An exclusion area can be incorporated to address for non-relevant changes. 
...
The datasets analyzed during the study are available in the corresponding databases or repositories (\url{https://covid19nowcasthub.de/}, \url{https://github.com/bahmanrostamitabar/hourly-emergency-care}, and \url{https://physionet.org/content/mimic3wdb/1.0/}).
All code is available in the repository \url{https://github.com/jo-rie/aatc}.
...
Typically, the percentile approach requires larger datasets and provides an easy and fast estimate, while the \ac{bca}, though computationally expensive, can produce reasonable confidence intervals with smaller datasets.
