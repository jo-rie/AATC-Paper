...
We refer to time steps for which either \ac{mnf} or true values, or both, are unavailable as missing values.
Systematical missing values could lead to a biased assessments, and missing data should be inspected for any underlying patterns.
...
Points in the green upper right and lower left quadrants reflect a correct change direction for the respective time step, whereas points in the remaining red quadrants show incorrect predicted changes.
Figure~\ref{fig:aatc_basic_4q} displays a basic four-quadrant plot and
Figure~\ref{fig:aatc_basic_4q_sample} shows a four-quadrant graph for simulated data with $T=1461$, for example, four years of daily data.
...
\caption{Four-quadrant plot with rectangular exclusion area.}\label{fig:aatc_basic_4q_excl_box}
\end{subfigure}\hspace{0.01\textwidth}%
\begin{subfigure}[t]{.24\textwidth}
\includegraphics{plots/illustrative_examples/4q_excl_axis}
\caption{Four-quadrant plot with horizontal exclusion area.} \label{fig:aatc_basic_4q_excl_axis}
\end{subfigure}\hspace{0.01\textwidth}%
\begin{subfigure}[t]{.24\textwidth}
\includegraphics{plots/illustrative_examples/4q_excl_cross}
\caption{Four-quadrant plot with cross-shaped exclusion area.}\label{fig:aatc_basic_4q_excl_cross}
\end{subfigure}%
\caption{Illustrations of the four-quadrant plot with sample points and with and without exclusion areas. In Figure~\ref{fig:aatc_basic_4q}, points 2, 3, 5, and 6 correctly track change, whereas points 1, 4, and 7 do not.}
\label{fig:aatc_4q}
\end{figure}
...
With that, we mainly estimate the probability of a correctly predicted change direction, $P(\diffxrv \diffyrv > 0)$, where $\diffyrv$ and $\diffxrv$ denote random variables for future incremental changes.
...
Two simple measures that focus on either a positive or negative predicted change are the positive and negative \ac{atc} ratios $\accp$ and $\accm$, respectively.
They are defined as
\begin{align}
...
The ATC ratio has a strong sinus-shaped seasonality with a peak after a quarter of a year and a low point after three quarters.
}
\end{figure}
...
In that case, a point's assignment to a quadrant can be driven by noise instead of a systematic \ac{atc}.
...
Figure~\ref{fig:aatc_4q} visualizes examples of exclusion area shapes.
...
Points are included if at least one of true value or prediction is unlikely to be noise-driven.
...
This particularly suits \ac{mnf} where small amounts of noise are inevitable.
...
A still finer analysis might be gained by considering the conditional distribution $P(\diffxrv \diffyrv > 0 | \diffxrv = \xcond)$ to assess the \ac{atc} ability of a prediction for a certain change $\diffxrv = \xcond$ of the \ac{mnf}.
...
Furthermore, the graph facilitates the comparison of various methods in a single graph, and asymmetries of $P(\diffxrv \diffyrv > 0 | \diffx = \xcond)$ with respect to $x$ in the \ac{atc} can be detected.
...
For \ac{atc} analysis, probabilistic predictions thus also contain a probability of a positive or negative change.
In this section, we extend the \ac{atc} assessment to probabilistic predictions.
...
In Appendix~\ref{subsec:app-probabilistic-atc-evaluation}, we extend the analysis for predictions given as quantiles or unknown true values.
...
\end{equation*}
The probabilistic \ac{atc} evaluation is then based on the probabilities $\mathbf{p} = (p_t)_{t=l}^{T}$ and the observed changes $\mathbf{z} = (z_t)_{t=l}^{T}$.
...
\end{equation*}
Thereby, lower values indicate a higher probabilistic \ac{atc} ability of the considered method.
...
In this section, we evaluate the \ac{atc} in three different applications.
...
Section~\ref{sec:application-eda} applies the \ac{atc} assessment to forecast methods for the number of arrivals in a large emergency department.
In the last application in Section~\ref{sec:application_measurement}, we consider the \ac{atc} assessment of non-invasive blood pressure measurements compared to invasive blood pressure measurements.
...
In addition to close inspection of the point evaluation measures, assessing the \ac{atc} of the nowcasts is crucial.
...
In the following, we apply the \ac{atc} assessment of Section~\ref{sec:aatc} to the nowcasts of the seven-day hospitalization rate.
We report the \ac{atc} for the horizons one, seven, and 14 days.
While horizon one assesses the short-term \ac{atc}, horizons seven and 14 consider the medium-term \ac{atc}.
...
The 10\%-quantile is used for the exclusion areas in the \ac{atc} assessment.
...
Table~\ref{tab:app-covid-atc-ratios-lag-7} lists the \ac{atc} ratios for all models with and without exclusion areas for the horizon of seven days.
...
The positive \ac{atc} ratio implies a similar ranking of the models, while the negative ratio is second best for the RKI model.
...
In general, the \ac{atc} ratio increases with larger exclusion areas.
...
Figure~\ref{fig:app-covid-atc-ratio-7} shows that the \ac{atc} ratio increases with larger exclusion areas.
...
From the issued forecasts, we use the mean as a point forecast for the \ac{atc} assessment and evaluate the probabilistic \ac{atc} subsequently.
...
For some models, for example, the GBM-2 model, the positive \ac{atc} ratio is higher; for some models, for example, the tbats model, the negative \ac{atc} ratio is higher.
...
\begin{figure}
    ...
    \end{subfigure}
    \caption[Conditional ATC plots for the horizons three and seven days and the models with the best positive or negative \ac{atc} ability in emergency department arrival forecasting.]{Conditional \ac{atc} plots for the horizons three and seven days and the models with the best positive or negative \ac{atc} ability in emergency department arrival forecasting. The plots of NBI-2 and Poisson-2 are indistinguishable.}
...
The models' \ac{atc} is generally higher for the smaller horizon, but the differences are minor, and confidence intervals overlap.

The \ac{atc} differs for all models for positive and negative predicted change directions.
...
The results of the probabilistic \ac{atc} assessment endorse the point \ac{atc} assessment and assign the best scores to NBI-2 and Poisson-2.
...
Again, we exclude the smallest 10\% of absolute differences in \ac{atc} assessment.
...
It becomes apparent that the systolic measurements have a higher \ac{atc} ability than the mean measurement, except for small negative predicted changes.
This aligns with the \ac{atc} ratios, but the confidence intervals overlap.
...
The positive and negative \ac{atc} ratios have overlapping confidence intervals, and the conditional \ac{atc} plots do not contain prominent deviations in the course.
...
In this paper, we examine various methods to assess the \acf{atc} for \acf{mnf}, that is, whether predict the correct direction of changes.
...
The estimators of Section~\ref{subsec:atc-measures} do not account for imbalances in the number of observed positive and negative changes \citep[for a theoretical analysis, see][Chapter 3]{Jolliffe2012}.
...
\begin{table}
    ...
    \begin{subtable}[t]{\textwidth}
        ...
        \caption{14 days.}
    \end{subtable}
    \caption[ATC ratios for the models without and with exclusion areas for the horizon one and 14 days in Covid nowcasting.]{\Ac{atc} ratio $\acc$, positive \ac{atc} ratio $\accp$, and negative \ac{atc} ratio $\accm$ for the models without and with exclusion areas for the horizon one and 14 days in Covid nowcasting. The exclusion areas are rectangles centered on the zero points with a width and height of 10\% of the quantile of the absolute values of nowcast and true values. }
