...
\abstract{
An intuitive and easily interpretable information on a quantity measured over time is whether it increases or decreases.
If the quantity of interest is not yet accessible, we refer to the ability to accurately predict, nowcast, or measure the direction of change for a true variable as trending. 
An appropriate assessment of the trending ability is crucial for decision-makers, as it enables them to assess the method's credibility and changes in the underlying process early.
While the current practice of evaluating signals is focused on averaging over local deviation measures, we present trending evaluation techniques as accompanying assessments that consider whether the development direction fits the quantity of interest. 
...
We review and present new visualization techniques to assess the local trending behavior and enlarge the scope to probabilistic nowcasts and forecasts. 
We apply the methods to three large medical data applications: COVID-19-nowcasting, emergency department arrival forecasting, and invasive and non-invasive blood pressure measurements.
}
...
\section{Introduction}\label{sec:trending-introduction}
...
As discussed in Section~\ref{sec:introduction}, the term \enquote{trending} denotes the alignment of a detected true development with the development indicated by a measurement, a nowcast, or a forecast.
...
Similar evaluations are used in other scientific areas, for example, with contingency tables in dichotomous forecasting or with confusion matrix in classification analysis~\citetext{see, e.g., the introductions in \citealt{James2021}, Ch. 4, and \citealp{Jolliffe2012}, Ch. 3}.
...
Points are included if the true value or signal is unlikely to be noise-driven.
...
Thereby, $F_{t | t-l} (x)$ yields the probability that the quantity of interest is at most $x$.
...
The \ac{cdf} $F_{t | t-l} (y_{t-l})$ yields the predicted probability of any value at most $y_{t-l}$, a negative change.
Thus, $1 - F_{t | t-l} (y_{t-l})$ is the probability of a positive change.
...
In addition to those nowcasts, two ensemble methods are constructed using the ensembles' mean or median.
...
\begin{table}[]
    ...
    \caption[The trending ratios for Covid nowcasting.]{Point evaluation measures for the issued mean of the different models. The evaluation period comprises 159 days. }
...
\begin{table}
    ...
    \caption[The trending ratios for Covid nowcasting.]{The trending ratio $\accl[7]$, positive trending ratio $\accpl[7]$, and negative trending ratio $\accml[7]$ for the models without and with exclusion areas for the horizon seven days. The exclusion areas are rectangles centered on the zero points with a width and height of twice the 10\%-quantile of the absolute values of nowcast and true values. The subscript $q_{0.1}$ denotes the measures with exclusion area. }
...
%    \includegraphics{}
    ...
    \end{subfigure}
    \caption[Conditional trending plot and trending ratio over exclusion area for the Covid nowcasts.]{Conditional trending plot and trending ratio over exclusion area for the nowcasts of the seven-day hospitalization rate ILM, RKI, RIVM, and ENS-MED for the horizon seven days.}
...
Remember that low a \ac{bs} and a plot along the diagonal are signs of good trending.
The \ac{bs} is the lowest for the RIVM model, while the ENS-MED model has the best \ac{bs} for the horizon of seven and 14 days.
The RKI model yields the highest \ac{bs} for all horizons.
Note that the \ac{bs} is bounded by 0.25 for random guessing; thus, all models perform better than random guessing.
...
For the horizons of seven and 14 days, the nowcasters issue only a few moderate probabilities.
...
\begin{figure}
    ...
    \end{subfigure}\hspace{0.01\textwidth}%
    \caption[Brier Scores and reliability diagrams for ILM, RKI, RIVM, and ENS-MED.]{\aclp{bs} and reliability diagrams for ILM, RKI, RIVM, and ENS-MED.
...
Thus, the reliability diagrams for the horizons of 7 and 14 days do not contain information on moderate probabilities.
...
\caption[Point evaluation measures for the emergency department arrival forecasting models.]{Point evaluation measures for the models. The smaller count for some models stems from missing forecasts scattered throughout the evaluation period.}\label{tab:app-eda-point-evaluation}
\end{table}
...
Thus, we exclude differences smaller than one from the trending assessment.
...
\begin{table}
    ...
    \caption[Trending ratios for the emergency deparment arrival forecasts.]{Trending ratio $\acc$, positive trending ratio $\accp$, and negative trending ratio $\accm$ for the models with the exclusion of zero-containing points for the horizons 72 hours and seven days.}
...
\begin{figure}
    ...
    \end{subfigure}
    \caption[Conditional trending plots for the horizons three and seven days and the models with the best positive or negative trending ability in emergency department arrival forecasting.]{Conditional trending plots for the horizons three and seven days and the models with the best positive or negative trending ability. The plots of NBI-2 and Poisson-2 are indistinguishable.}
...
\begin{table}
    ...
    \begin{tabular}{l l}
        ...
        \bottomrule
    \end{tabular}
    \caption[Matching the abbreviation to the key in the nowcasting hub.]{Matching the abbreviation to the key in the nowcasting hub.
...
\begin{table}
    ...
    \caption[Marginal analysis of the Covid nowcast and true differences for the horizons 1, 7, and 14 days.]{Marginal analysis of the nowcast and true differences for the horizons 1, 7, and 14 days.
...
\begin{table}
    ...
    \begin{subtable}[t]{\textwidth}
        ...
        \caption{14 days.}
    \end{subtable}
    \caption[Trending ratios for the models with and without exclusion areas for the horizon 1 and 14 days in Covid nowcasting.]{Trending ratio $\acc$, positive trending ratio $\accp$, and negative trending ratio $\accm$ for the models with and without exclusion areas for the horizon 1 and 14 days. The exclusion areas are rectangles centered on the zero points with a width and height of 10\% of the quantile of the absolute values of nowcast and true values. }
...
\caption[Four-quadrant plots for the Covid nowcast models ILM, RIVM, RKI, and ENS-MEAN and the horizons of one, seven, and 14 days.]{Four-quadrant plots for the models ILM, RIVM, RKI, and ENS-MEAN and the horizons of one, seven, and 14 days. The spread in both directions increases with the horizon. }
...
%    \includegraphics{}
    ...
    \end{subfigure}
    \caption[Conditional trending plot and trending ratio over exclusion area in Covid nowcasting.]{Conditional trending plot and trending ratio over exclusion area for the nowcasts of the seven-day hospitalization rate ILM, RKI, RIVM, and ENS-MED for the horizon seven days.}
...
\begin{figure}
    ...
    \end{subfigure}
    \caption[Reliability diagrams for Covid nowcasting models and the horizon seven and 14 days.]{The reliability diagram for the models ILM, RIVM, RKI, and ENS-MED for the horizon seven and 14 days.
