...
% XXX should we delegate that table to the appendix as before?
...
\begin{table}
    ...
    \caption[ATC ratios for the forecasts of patient admission to an emergency department.]{\Ac{atc} ratio $\acc$, positive \ac{atc} ratio $\accp$, and negative \ac{atc} ratio $\accm$ for the models with the exclusion of zero-containing points for the horizons 72 hours and seven days in the forecasting of patient admissions to an emergency department.}
    \label{tab:app-eda-atc-ratios}
\end{table}

% XXX in the figure above: comment on the exclusion area; notation in columns is not consistent to the example anbove (with respect to using an exclusion area a or not), I think; i.e., \mu_q vs \mu
...
\end{table}
% XXX hier wieder die Sache mit der Notation und Exclusion Zone
...
% XXX should we exclude the 1 min horizon, i.e., does it make sense to look at such a small horizon? (especially also because of the large CIs? Though that might be interesting for didactic reasons)? The NBP will take some time, so I am not sure how the exact time of the measurement is chosen
