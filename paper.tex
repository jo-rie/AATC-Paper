...
We refer to the ability to accurately predict, nowcast, or measure the direction of change of a true variable (value???) as "trending". 
Trending describes the ability of a method to measure or predict the right direction [and magnitude???] of changes in a true variable (value???). Currently, evaluating signals to assess trending  is focused on averaging over local deviation measures. We here present techniques to assess trending as accompanying assessments that consider whether the development direction [direction of change???] fits the quantity of interest. 
...
We exemplarily apply the methods to three medical or healthcare problems: COVID-19-nowcasting, emergency department arrival forecasting, and invasive and non-invasive blood pressure measurements.
}
...
Conventionally, differences between predictions (or measurements) and target variables (values???), such as the \ac{rmse} or \ac{mae}, form the basis of method evaluation.
These measures do not consider trending, that is, the ability of a method to measure or predict the right direction [and magnitude???] of changes in a true variable (value???). 
However, the information about an increase or decrease in a true variable (value???) is crucial to detect changes in underlying processes.
...
The aspect of trending is not considered in the literature to the best of our knowledge.
...
In this article, we formalize trending and present different variants of trending measures that consider either noiseless data or data with noise and small non-informative changes.
...
\begin{table}
    ...
    \begin{subtable}[t]{\textwidth}
        ...
        \caption{14 days.}
    \end{subtable}
    \caption[Trending ratios for the models with and without exclusion areas for the horizon one and 14 days in Covid nowcasting.]{Trending ratio $\acc$, positive trending ratio $\accp$, and negative trending ratio $\accm$ for the models with and without exclusion areas for the horizon 1 and 14 days in Covid nowcasting. The exclusion areas are rectangles centered on the zero points with a width and height of 10\% of the quantile of the absolute values of nowcast and true values. }
